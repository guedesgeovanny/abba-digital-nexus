...
-- Inserir conversas de exemplo diretamente no banco
-- Versão corrigida com declaração adequada da variável de loop
...
BEGIN
    ...
    FOR user_record IN SELECT id FROM auth.users LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM public.conversations WHERE user_id = user_record.id) THEN
            ...
            SELECT c.id, m.content, m.direction::message_direction, m.message_type::message_type, m.sender_name, m.created_at
